...
| `packages/url4` | `url4-python-sdk` | Python · uv · library | `uv run pytest` · `uv run ruff check` · `uv run pyright` | `url4-tests.yml` | release-please → `url4-v*` → `release-url4.yml` (PyPI, Trusted Publishing) | Protocol, parser, AST, and abstract streaming boundaries stay free of concrete app adapters. |
