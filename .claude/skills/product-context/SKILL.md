...
|---|---|---|---|
| The ScreamingFace one-liner says we combine models into "fusion models", while the Fusion entry forbids exactly that phrase and the claim it implies. Canon contradicts itself on the core term | **open** | brand | terminology, `ScreamingFace` one-liner vs. `Fusion` entry |
| Public claim language for benchmarks we authored has not been approved | **open** | claims | terminology, `Benchmark` entry: "Ronnie has not yet approved public claim language for authored benchmarks" |
| The external name of the community program is not final | **provisional** | brand and GTM | terminology, `Fusion Monsters` entry: "the external program name is not final (\"Fusion Monsters\" vs \"Scream Team\")" |
| The persona set is still being refined | **provisional** | brand | personas file preamble: "These audiences are still being refined, not all are final" |
| The top terminology entry is marked as needing review | **provisional** | brand | terminology, `ScreamingFace` entry, `Status: Needs Review` |
| Whether the heavy-user tier of the token-payer persona should split into its own entry | **open** | brand | personas file, `Everyday AI Users / General Token Payers` entry: "should the heavy-user tier be split into its own persona?" |
