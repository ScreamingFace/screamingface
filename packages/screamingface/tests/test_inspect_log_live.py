...
def test_unscored_report_exports_as_error_status_and_reopens(tmp_path: Path) -> None:
    # WHY this exists live and not only in the pure tests: status="error" is the
    # one document shape the success-path test never pushes through inspect's
    # REAL validator, and that surface is exactly version-shaped (the pin's job).
    from inspect_ai.log import read_eval_log
    ...
    unscored = sf.CandidateResult(
        ...
        members=(),
        failures=(),
        usage=sf.Usage(input_tokens=100, output_tokens=0, cost_usd=None),
    )
    value = sf.Report(benchmark=benchmark, case_count=2, candidates=(unscored,))
