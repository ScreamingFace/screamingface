...
# --- OME-993: collected error rows carry the failure's code + permanence -------------


@pytest.mark.asyncio
async def test_collected_error_row_carries_code_and_permanence() -> None:
    # INVARIANT: `on_error=collect` must not strip what downstream consumers need to
    # propagate the ORIGINAL failure — a Url4Error's `code` and `permanent` ride the
    # error row so a benchmark fold can re-raise the real cause (never a shape
    # misdiagnosis) and callers can decide retryability.
    from url4.core.errors import ResolutionError

    def _judge(context: str, intent: str) -> str:
        raise ResolutionError(
            "aigateway request failed with status 429",
            code="aigateway_http_429",
            permanent=False,
        )

    io = StaticIOLayer(
        fetch_map={"https://rows": '[{"q": "a"}, {"q": "b"}]'},
        routes={"/judge": _judge},
    )
    result = await run("https://rows*(r:0:/judge($item.q)!'go')!'$r'", io)
...
@pytest.mark.asyncio
async def test_collected_error_row_for_a_plain_exception_keeps_the_lean_shape() -> None:
    # A non-Url4Error has no code/permanence to report — the row keeps the
    # pre-OME-993 two-field shape rather than inventing fields.
    def _boom(context: str, intent: str) -> str:
        raise ValueError("boom")

    io = StaticIOLayer(
        fetch_map={"https://rows": '[{"q": "a"}]'},
        routes={"/judge": _boom},
    )
    result = await run("https://rows*(r:0:/judge($item.q)!'go')!'$r'", io)
    rows = json.loads(result)
    assert rows == [{"error": {"kind": "ValueError", "message": "boom"}}]
