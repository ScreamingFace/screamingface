...
the :class:`LoweringRegistry` and its per-node lowerers (parse-tree atom →
executable node), plus the text and parse-tree decoding that feeds them. The
group-wiring strategy (slot → graph shape) lives in :mod:`url4.dag._wiring`;
