...
def _case_result(
    ...
    A decision ladder, most-broken first — each rung becomes a Failure whose ``code`` makes a
    ``None`` exam score traceable per Case:

        no rubric asset      -> "missing_rubric_asset"
        no row for this Case -> "missing_case_row"
        row is an error row  -> "case_error"
        verdicts incomplete  -> "incomplete_verdicts"
        complete, no + item  -> "no_positive_points" (a baked-asset defect: `prepare` guarantees
                                at least one positive criterion survives the container filter)
    """
...
def _terminal_failure_outcome(
    ...
    if points is None:
        ...
        # mid-chain error surfaces here as a missing row. Without the orphan payloads the report
...
def _failed_result(
    ...
    fields = _candidate_fields(row)
    # WHY a grade with score None rather than no grade: the judge evidence for a partially
    # judged Case is audit material, and the grade's checks list is the contract's slot for it.
