...
JUDGE_PARAMS = (
    ...
    # thinking and return a blank `length` turn, billing tokens while failing the verdict. A
    # MET/UNMET call against one written criterion does not need deep reasoning; low keeps
    # ~4.5k judge calls per candidate cheap.
    ("reasoning_effort", "low"),
    # WHY 8192 where DRACO holds 4096: DRACO's bound reproduces its paper's parametrization;
    # GDPval has no paper judge to reproduce (see WHY above), so the bound is ours to set and
    # doubling it prefers a completed verdict over an occasional token-cap failure. Still an
    # engine-side safety stop — a verdict is a sentence and a boolean.
    ("max_tokens", "8192"),
)
