...
_CURRENT: ContextVar[tuple[RunObservations | None, CaseId | None] | None] = ContextVar(
    "benchmark_case", default=None
)


@contextmanager
def case_scope(case_id: CaseId | None) -> Iterator[None]:
    """Carry explicit metadata through nested work; unlabelled calls mask outer cases."""
    token = _CURRENT.set((current_observations(), validate_case_id(case_id, optional=True)))
    try:
        yield
    finally:
