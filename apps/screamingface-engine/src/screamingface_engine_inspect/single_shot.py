...
_FAILURE_MESSAGES: Mapping[str, str] = {
    "scorer_error": "the imported scorer raised while grading this Case",
    "invalid_score_value": "the imported scorer returned a value this board cannot map to a score",
...
class ImportedBoard:
    ...
    def scored_path(self) -> ScoredPath:
        ...
        return ScoredPath(
            ...
            grading_failure_message="the imported scorer pipeline could not grade this Case",
    ...
    benchmark = Benchmark(
        ...
        check_surface=(
            CheckSurface(
                ...
                # Free: both proof scorers are deterministic. A model-graded
                # import declares "paid" at its own assembly call (OME-1116).
...
def _check(root: Path) -> Callable[[Request], str]:
    ...
    def check(request: Request) -> str:
        try:
            ...
            _target(root, case_id)  # refuse early when the private asset is unusable
...
def check_surface_verdict(
    ...
    if outcome.failure_code is not None:
        raise ValueError(_FAILURE_MESSAGES[outcome.failure_code])
