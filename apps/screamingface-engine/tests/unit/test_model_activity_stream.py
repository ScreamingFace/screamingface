...
async def publish(handler, *, level="full", expression="/model('PRIVATE PROMPT')!go"):
    async with httpx.AsyncClient(
        ...
        executor = build_executor(
            ...
            config(),
            client=client,
            observers=observation_factories({"URL4_CLOUD_ACTIVITY_LEVEL": level}),
        )
...
def test_unknown_deployment_policy_fails_at_composition():
    with pytest.raises(ValueError):
        observation_factories({"URL4_CLOUD_ACTIVITY_LEVEL": "aggregate"})
...
def test_real_model_activity_survives_hosted_websocket_pump():
    ...
    runner = InProcessJobRunner(
        stream,
        lambda env: build_executor(
            env, config(), client=http_client, observers=observation_factories(env)
        ),
