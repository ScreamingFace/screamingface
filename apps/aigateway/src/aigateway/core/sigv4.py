"""AWS Signature Version 4 — the narrow slice needed to PUT one object (OME-1021).

Ported from the Engine's `artifacts/sigv4.py` (OME-929) by copy, not import: the two apps
must not depend on each other, and the signing math is pure and already pinned against
AWS's published vectors — duplicating it is cheaper and safer than coupling the packages.

WHY hand-rolled: the whole requirement is PUT of a single object against our own
S3-compatible endpoint. No multipart, no presigning, no chunked payload signing. That
fits in pure functions and keeps `httpx` the only HTTP dependency.

WHY that is safe: we SIGN and the server VERIFIES. A defect here makes our own request
fail with 403; it cannot make a forged request acceptable. The failure mode is closed,
which is what makes hand-rolling defensible at all.

INVARIANT (the bound): this stays PUT of a single object. Multipart, presigned URLs, or
chunked payload signing are the signal to take a real S3 client instead of growing this.

Everything here is pure — no I/O, no ambient clock. `now` is a parameter so the signature
is reproducible in a test, which is what lets it be checked against AWS's published vectors.
"""
...
def canonical_request(
    *,
    ...
    are path-safe lower-case names (`cache-snapshots/<stamp>.sql.gz`) — nothing to encode,
    and re-encoding a key that needs none is a silent 403.
    """
