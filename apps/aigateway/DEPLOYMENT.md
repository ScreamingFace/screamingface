...
`request_cache_entries`. The column is nullable with no default. On Postgres the operation changes
only the catalog: no table rewrite, no long lock, no backfill. On SQLite it is a plain in-place
`ADD COLUMN`.
