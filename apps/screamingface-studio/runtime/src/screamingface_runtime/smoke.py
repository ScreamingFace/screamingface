...
async def smoke(engine_url: str, gateway_url: str) -> int:
    """Verify both services and the Engine-to-Gateway catalog integration."""

    async with httpx.AsyncClient(timeout=30.0) as http:
        for service, base_url in (("Engine", engine_url), ("AI Gateway", gateway_url)):
...
def main(argv: list[str] | None = None) -> None:
    ...
    model_count = asyncio.run(smoke(args.engine_url, args.gateway_url))
