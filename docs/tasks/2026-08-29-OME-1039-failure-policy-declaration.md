...
`healthbench/aggregate.py` into the new `benchmarks/spine/grading.py` — the first
...
`finalize_candidate_result`, which is uniform coverage_declare behavior — so all six
builtins declare `coverage_declare`; `withhold` is a valid enum value with no current
