...
Concrete mechanics are now proposed for review: producer observation timestamps with a documented 30-second relative clock assumption and conservative age deadline; one 100/s, burst-200 bucket reserving 40 tokens from routine starts/heartbeats; and a 4,096-ID / 1 MiB collision-check window with explicit post-eviction behavior. The freshness estimate can warn up to 60 seconds early under the clock assumption; unknown clocks do not justify extrapolation. See the [mechanics ledger](../work/2026-09-10-OME-887-concrete-activity-contract.md). These proposals do not authorize implementation.
