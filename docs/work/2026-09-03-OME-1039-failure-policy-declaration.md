...
uniform **coverage-declare** behavior. The declaration must tell the truth (that is the
whole point of the ticket), so **all six builtins declare `coverage-declare`**; `withhold`
...
- `src/screamingface_engine/benchmarks/definition.py` — add `BenchmarkDeclaration`
  (frozen dataclass: `failure_policy: "withhold"|"coverage-declare"`,
...
- `src/screamingface_engine/benchmarks/spine/case_ladder.py` (new) — `CaseLadder` class
...
- `src/screamingface_engine/benchmarks/gdpval/aggregate.py` — build module-level
  `CaseLadder`, delete the 8 copied functions.
...
- Board definitions add declarations: `ifeval/definition.py`, `gdpval/exam.py`,
  `healthbench/exam.py`, `draco/exam.py` (all `coverage-declare` + `single_shot`).
...
- `tests/unit/test_benchmark_declaration.py` (new):
  ...
  - every builtin declares; all six pinned `coverage-declare` (invariant: declaration
    matches `finalize_candidate_result` reality)
- `tests/unit/test_spine_case_ladder.py` (new): one test per rung through the shared
...
- **Actual files:** as planned, plus `benchmarks/__init__.py` (re-export
  ...
  New tests: `test_benchmark_declaration.py` (8), `test_spine_case_ladder.py` (8).
...
- **Deviations:**
  - Ticket's Before-narrative (ifeval/draco "all-or-nothing") is stale: all four boards
    reduce through shared `finalize_candidate_result` = uniform coverage-declare. All six
    builtins declare `coverage-declare`; `withhold` has no claimant. Linear comment posted.
