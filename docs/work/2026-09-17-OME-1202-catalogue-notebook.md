...
- **Late addition — a seeded run in section 3.** Once OME-1227 (`sf.evaluate(answer_seed=…)`) and OME-1231 (the preflight that refuses a seed the provider cannot honour) merged into this branch, the owner exercised both in the notebook and asked for the result folded back into the generator. Two cells now re-run the gsm8k panel at `answer_seed=42` and `answer_seed=7`, and the synthesiser moved from `claude-haiku-4.5` to `qwen3.8-flash` — **not** cosmetic: Anthropic's Messages API has no seed field, so the preflight refuses the seeded call outright and an Anthropic synthesiser would make the new cells raise. A `WHY` in `build_notebooks.py` pins that reason to the model list so a future edit does not quietly swap it back. Owner's paid runs of both cells: score 1.0 / coverage 1.0 each, $0.002329 and $0.002131 metered, `answer_seed` 42 and 7 recorded on the candidate. Note those two runs show the seed *changing* the sample, not *pinning* it — proving reproducibility needs two runs at the SAME seed, which was not spent.
