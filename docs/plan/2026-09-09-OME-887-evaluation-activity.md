...
- Implement the proposed single 200-token bucket refilling at 100/s, reserving 40 tokens from started/running records for terminal/retry observations. Test the exact 40/41-token boundary, empty/full bucket, concurrent admission, refill and invalid-record handling; no second budget. Test saturation and recovery, including later valid records after an oversized record; no optional logging limit may block or fail work.
- Verify producer observation timestamps precede sink/bridge buffering and remain unchanged in replay; all timestamps remain subject to full/off policy. Real fake-provider completion, refusal, local transport retry, safe failure, cancellation and heartbeat timing. Use a simulated clock to verify fixed 60-second ticks across multi-day durations, no doubling or catch-up bursts, and immediate observed outcomes independent of timer phase. Preserve the number of calls, retry delays and original errors.
...
- Bound `_RunState._event_ids` before the Logs projection: proposed 4,096 IDs / 1 MiB window plus constant-size sequence cursor. Document the explicit bounded-window contract: reject reused IDs retained in the window, accept otherwise valid new sequences with evicted IDs, and always suppress already accepted sequences; preserve replay suppression, gap detection and authoritative event behavior. Exercise oversized tracking entries, duplicates within/outside the window, more than 20,000 events and multi-day synthetic streams. Measure decoder and projection state independently.
...
- Use the owner-approved 180-second threshold since the last fresh update, not operation start. Test a 30-minute call with fresh 60-second heartbeats: it stays active throughout. Test dropped terminal records during an ongoing run: freeze elapsed time and show “No recent update; call may still be running” (or “operation” for other work), never failure. Establish actual fresh-observation evidence before resuming timers; replay receipt, revision advancement and connection heartbeat alone do not qualify. Use producer `sf.activity.observed_at_ms` unchanged through relay, the spec's conservative 30-second clock allowance, age-adjusted monotonic deadline and post-attach observation rule. Test delayed ordinary delivery, queued/replayed higher revisions, both extremes of the allowed clock offset (including the conservative warning up to 60 seconds early), 40-second future timestamps, backward source timestamps, clock jumps, absent clock assumption and late terminals. Unknown clock validity means no extrapolation; no new live/replay transport marker.
