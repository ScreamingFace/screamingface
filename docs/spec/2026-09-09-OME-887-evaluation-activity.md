...
| `sf.activity.observed_at_ms` | UTC Unix milliseconds captured by the producer when constructing this observation, before sink/bridge admission; nonnegative safe integer, immutable through relay/replay |
...
- Per-run activity rate: token bucket at 100 records/s, burst 200. There is no lifetime limit on the number of emitted activity records; long runs remain eligible to emit throughout execution. Each operation has a constant-size current record, not retained history. Rate suppression leaves execution unchanged and does not cap authoritative spans/results. Tokens replenish continuously; a temporary burst does not disable later logging. Sustained overload can continue suppressing records. Favor terminal outcomes (including failures) and observed retries over repetitive running updates using bounded admission capacity within the same total rate/burst limits; no priority class bypasses size validation, bridge bounds or evaluation cancellation. Concrete proposed admission: refill a single bucket with monotonic time at 100 tokens/s up to 200. Each accepted record costs one token. `started` and `running` require at least 41 tokens before admission, leaving 40 reserved; `retrying` and terminal states require at least one. Refill/check/debit are atomic within the shared run session. Full initial capacity is 200. This reserves burst capacity for outcomes/retries without an additional rate budget; it does not guarantee admission under sustained high-priority traffic. Invalid/oversized records consume no token, and rejected records increment the applicable suppression counter once. No allocation by free-form severity or message text; do not add a waiting queue or delay the work to obtain a token. A total emission limit may be reconsidered separately if operational evidence warrants it.
...
This deliberately bounds cross-sequence ID-reuse detection to retained IDs. It does not guarantee lifetime uniqueness validation after eviction. Replay at already-accepted sequence numbers is still suppressed without retaining their IDs, and each newly accepted sequence follows the existing callback path. Reused IDs at new sequence numbers outside the window may no longer be detected; the proposed Client contract explicitly limits this check to the bounded window. Preserve existing error behavior for a reused ID still inside the window; accept an otherwise valid new sequence with an evicted ID. Update the contract documentation and add regression tests for both cases; do not silently remove the existing in-window rejection test. Do not claim lifetime exactly-once delivery by event ID. No approximate membership filter that can falsely discard authoritative events. Test multi-day streams through the actual decoder plus projection, including authoritative events, not only reducer fixtures.
...
An operation row represents the last observed state, not proof the work is still active. A fixed heartbeat cadence is not a completion guarantee. Owner-approved freshness threshold: 180 seconds since the last demonstrably fresh operation update (three intended heartbeat intervals), not since the operation started. A 30-minute model call receiving fresh 60-second heartbeats stays active throughout. After this threshold without a fresh update, show “No recent update; call may still be running” for a model call, or “No recent update; operation may still be running” for other work, and stop extrapolating elapsed time. This display state never times out, cancels or fails the actual operation. Preserve the last measured elapsed value and observation time; do not infer failure, completion, provider progress or a cancellation request. Apply this also to synchronous start records, which have no guaranteed intermediate heartbeat.
...
Concrete proposed freshness evidence: the producer stamps `sf.activity.observed_at_ms` at observation creation, before any queue. The current URL4 `Log` has no observation timestamp, so carry this scalar in the existing attributes; generic relay must not replace it with serialization, publication or replay time. Terminal elapsed duration continues to come from the producer's monotonic timer. OME-1161 owns stamping; OME-1135 owns interpretation. No new URL4 API, stream event or transport live/replay marker is required.

This is a UI freshness estimate under a documented maximum 30-second relative clock-offset assumption between producer UTC and Client UTC, not proof that work is still executing. A deployment/Client that cannot rely on that assumption disables elapsed extrapolation and shows measured historical observations with unknown freshness. Do not estimate clock offset from delayed stream events. An observation more than 30 seconds in the future, missing/invalid observation time, backward observation time at a higher revision, or a detected Client wall-clock jump invalidates freshness evidence without rejecting the underlying generic Log or affecting evaluation. Track Client wall/monotonic divergence; a change exceeding 30 seconds freezes active estimates for that connection. Do not restart them until a new connection has a usable clock assumption.

For an accepted new revision received at Client wall time `W` and monotonic time `M`, with producer observation time `O`, compute conservative age `A = max(0, W - O + 30_000)` milliseconds. It can drive an active display only if `A < 180_000`. Its deadline is `M + (180_000 - A)`, not receipt plus a fresh three minutes. Thus old delayed records and replay cannot gain a new full freshness window merely by arriving now. Repeated revisions do not update the deadline. The clock allowance can make the warning appear up to 60 seconds conservatively early under the allowed two-sided clock offset; it does not extend the approved threshold.

At initial attach and every reconnect, record Client wall time `C` and stop restored timers. A nonterminal observation may start an active timer only when `O > C + 30_000`, conservatively establishing that it was created after this attachment under the same clock assumption. Previously queued/replayed observations remain historical even when first seen or higher-revision. Await the next qualifying heartbeat/update rather than guessing a live boundary. For an eligible record, display the last measured elapsed value plus local monotonic time since its receipt until its conservative deadline; keep the observation-age estimate separate from measured duration. Terminal records can resolve rows regardless of age. Disconnect/run end always stops timers. This avoids historical heartbeat replay presenting as current activity, at the cost of a brief unknown state after attachment.

The clock allowance and attach rule are proposed implementation mechanics for owner review. If the clock assumption is unavailable, retain the truthful unknown-freshness display; no new time-sync service, persistent store, provider call or inferred provider progress is introduced.
