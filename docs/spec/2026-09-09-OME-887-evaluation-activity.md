...
- Display elapsed time from the last measured operation elapsed value plus local monotonic time while observation remains active; stop advancing on terminal observation, known run end or disconnection. Reconnection alone does not restart a timer from a stale summary; require fresh evidence for that operation. Label it as elapsed time, not provider progress. Show last received activity separately using local receipt time; a rendered timer tick must never reset that freshness marker or generate a synthetic Log.
