...
Optional suppression snapshot attributes are `sf.activity.suppressed.invalid`, `sf.activity.suppressed.oversize`, `sf.activity.suppressed.rate` and `sf.activity.suppressed.run_limit`. Each is a nonnegative integer, saturating at 9,007,199,254,740,991 (the JavaScript safe-integer maximum). Emit all four together as a cumulative run-local snapshot on the next admitted record after a counter changes. Consumers take per-reason maxima, never sum snapshots across records or replay. These counters cover producer suppression only, not sink or bridge loss.
...
- Per-run activity budget: token bucket at 100 records/s, burst 200, and at most 20,000 accepted activity records per run. Each operation still has a constant-size current record, not retained history. Runs exceeding the budget continue unchanged. This does not cap authoritative spans/results.
- Producer suppression counters: fixed reasons only (`invalid`, `oversize`, `rate`, `run_limit`), the saturation and snapshot fields specified above, no IDs/payload retention. Include snapshot bytes in the 4 KiB limit; reserve space before admission, with no recursive diagnostic emissions. After rate recovery, the next accepted record carries changed cumulative totals; at a permanently exhausted run budget no extra “guaranteed final Log” is promised. Client silence cannot establish completeness.
