...
`run_cost_usd` is **required on the request**. `run_cost_status` is **required** alongside it.
...
the same fact gets both, and then the board has to guess which one meant it.
